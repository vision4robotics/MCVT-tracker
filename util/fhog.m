...
[M,O] = gradientMag( I,0,0,0,1 );%I就是检测区域的像素值，M返回的是图像的梯度特征向量（大小和I相同，维度是1），O是近似梯度方向模（大小和I相同，维度是1）
H = gradientHist(M,O,binSize,nOrients,softBin,useHog,clip);%计算灰度直方图（32维）
